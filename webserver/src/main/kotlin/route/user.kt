    ...
    post<UserRoute> {
        ...
        call.respond(
            HttpStatusCode.Created, UserLoginResponse(
                user.toDTO(),
                authService.createJwtToken(user.snowflake)
            )
        )
    }
    get<UserRoute.Id> { (id) ->
...
class UserRoute {
    ...
    data class Me(val users: UserRoute) {
        @Resource("/works")
        data class Works(val me: Me) {
            @Resource("/{id}")
            data class Id(val id: Snowflake, val works: Works)
            @Resource("/mappings")
            data class Mappings(val works: Works) {
                @Resource("/{website}")
                data class Website(val website: String, val mappings: Mappings) {
                    @Resource("/{value}")
                    data class Value(val value: String, val website: Website)
                }
            }
        }
